...
- A completed run freezes an immutable `DiagnosticReport` in `RunAllTestsState`, owned by `RunAllTestsViewModel`. Opening an individual test from the report pushes its normal route; Back returns to the frozen report.
