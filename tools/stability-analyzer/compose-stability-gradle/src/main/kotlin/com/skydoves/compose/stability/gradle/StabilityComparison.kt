...
internal fun compareStability(
  ...
  current.keys.subtract(reference.keys).forEach { functionName ->
    if (!ignoreNonRegressiveChanges ||
      ...
      val entry = current.getValue(functionName)
      val parametersWithFixedStability = (entry.parameters + listOfNotNull(entry.receiver?.copy(name = "<receiver>")))
        .map { parameter ->
          parameter.copy(
  ...
  current.keys.intersect(reference.keys).forEach { functionName ->
    ...
    val receiver = currentEntry.receiver
    val oldReceiver = referenceEntry.receiver
    if (receiver != null && oldReceiver != null && receiver.stability != oldReceiver.stability &&
      (!ignoreNonRegressiveChanges || !receiver.isStable(forceStableTypes))
    ) {
      differences.add(StabilityDifference.ParameterStabilityChanged(
        functionName, "<receiver>", oldReceiver.stability, receiver.stability,
      ))
    }
...
private fun StabilityEntry.hasUnstableParameter(forceStableTypes: List<FqNameMatcher>): Boolean =
  parameters.any { !it.isStable(forceStableTypes) } || receiver?.isStable(forceStableTypes) == false
