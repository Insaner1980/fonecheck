...
public class StabilityInfoCollector(private val outputFile: File) {
  ...
  public fun export() {
    ...
    filteredComposables.forEach { entry ->
      require(identities.add(entry.canonicalIdentity())) {
        "Duplicate canonical stability identity: ${entry.canonicalIdentity()}"
      }
    }
    ...
    val report = StabilityReport(
      composables = filteredComposables.sortedBy { it.canonicalIdentity() },
    )
...
// Keep in sync with the Gradle report codec. Return type is not Kotlin overload identity.
internal fun ComposableStabilityInfo.canonicalIdentity(): String = qualifiedName +
  "(receiver=" + (receiver?.type?.let(::JsonPrimitive) ?: JsonNull) +
  "; params=" + JsonArray(parameters.map { JsonPrimitive(it.type) }) + ")"
...
public data class ComposableStabilityInfo(
  ...
  val receiver: ParameterStabilityInfo? = null,
)
