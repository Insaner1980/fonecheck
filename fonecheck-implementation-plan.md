...
- Tässä vaiheessa suunniteltu erillinen retest-finalisoija korvautui toteutetussa tuotantopolussa `RunAllTestsScreen`- ja `RunAllTestsViewModel`-omistajuudella: `RunAllSnapshotMapper` tuottaa snapshotit, `ReportAssembler` luo uuden `CATEGORY_ONLY`-raportin kohdekategoriasta ja `ReportRepository.insertOrConfirm` vahvistaa tallennuksen muuttamatta lähderaporttia. Task 25:n Report-detailin Retest-toiminto käyttää tätä RunAll-polkua; historiallista reporttia ei käytetä live-staten renderöintiin.
...
- Lisenssinäkymä näyttää nyt releaseen pakatun komponenttiluettelon ja Apache License 2.0 -ehdot; Compose-instrumentaatiotesti todentaa otsikon ja lisenssitekstin läsnäolon ja sen lähdekoodi kääntyy. Tuolloin valmis `store-assets/feature-graphic.png` poistettiin vanhentuneena 2026-09-23; uusi 1024 × 500 RGB PNG on valmisteltava ennen julkaisua. Aidot käyttöliittymäkuvat on edelleen otettava allekirjoitetusta Play-buildistä oikealla laitteella.
