...
| Localization inventory | English: 1,123 strings + 5 plurals; Finnish and Spanish: 1,122 strings + 5 plurals each; all three contain the same 1,127 translatable resource names; English additionally contains only the non-translatable `app_name` |
...
The settings language picker is separate from DataStore. `AppLanguage` has `SYSTEM` (`""`), `ENGLISH` (`"en"`), `FINNISH` (`"fi"`), and `SPANISH` (`"es"`). `SettingsRoute` reads the first locale from `AppCompatDelegate.getApplicationLocales()`, refreshes it on `ON_RESUME`, and writes selection through `AppCompatDelegate.setApplicationLocales(LocaleListCompat.forLanguageTags(...))`. AppCompat's disabled metadata-holder service has `autoStoreLocales=true`, so it owns language persistence on supported older Android versions; `AppPreferencesRepository` continues to store only theme, warnings, and onboarding.

`AndroidManifest.xml` points `android:localeConfig` to `res/xml/locales_config.xml`, whose entries are `en`, `fi`, and `es`. `MainActivity` handles `locale|layoutDirection` configuration changes, the app declares RTL support, and bundle language splitting is disabled so all three locales ship in the installed artifact. The manifest also removes the default EmojiCompat Startup initializer. A locale change must therefore be reviewed across activity recomposition/recreation, resource-backed route titles, external PDF formatting, RTL, and the system-level app-language UI; source presence alone does not prove every OEM/API path.
...
English is the source in `app/src/main/res/values/strings.xml`; Finnish and general Spanish ship in `values-fi/strings.xml` and `values-es/strings.xml`. Each locale has the same 1,127 translatable resources: 1,122 strings and 5 plurals. English additionally defines the non-translatable `app_name`. There are no string arrays or styled/XLIFF text resources in the current app source sets. Spanish plurals provide `one`, `many`, and `other`; the existing call sites pass the numeric quantity separately from the formatted count. Third-party notices remain in their original language, and platform/vendor values and unknown future stable-code fallbacks are not translated app copy. The language autonym `Español` is present in all three resource sets; the shared `readout_scroll_hint` is also localized in all three. Route titles, onboarding, settings, all diagnostic categories, guided checks and retests, accessibility, permissions, reasons, history, comparison, report presentation, and PDF text use the same resource path.
...
The in-app picker exposes System, English, Finnish, and Español through AppCompat. `AppLanguage.SPANISH` uses `es`; regional Spanish selections such as `es-ES` and `es-MX` resolve to the same general Spanish resources. `locales_config.xml`, the EN/FI/ES resource parity, AppCompat locale storage, `MainActivity` configuration handling, disabled bundle language splitting, and the Settings selection state form one feature boundary: changing one without the others can produce a visible option whose resources are absent, a resource language that cannot be selected, or a selection that does not survive restart.

Spanish follows the existing UI-language-only number and file-size policy: regional tags normalize to `es`, without a new regional-format setting. Human-readable dates use the existing localized medium formatter; technical timestamps keep the existing fixed format. `ReportPdfRenderer` obtains `ContextCompat.getContextForLanguage` at each render and uses that context for both content and page labels. Saved evidence, scores, comparison logic, JSON, and identifiers remain locale-independent.
...
ResourceParityTest now covers Spanish keys, format arguments, markup, line breaks, plural forms, duplicates, and category-font glyphs. Focused tests cover regional selection, numbers/dates, resource resolution, accessibility labels, PDF language changes, and JSON invariance. These JVM/instrumented tests were added but not executed: Gradle is not authorized. Local XML/contract checks are separate from Android resource compilation. Pending runtime validation includes Spanish on a differently localized device, app-language restart/system reset, representative screens at 320 dp and 200% font scale in both themes, TalkBack, and PDF wrapping with accented text and long observation reasons.
