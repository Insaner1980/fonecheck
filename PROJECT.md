...
| Localization inventory | Recounted for the Norwegian Bokmål addition on 2026-09-09: English, Finnish, Spanish, Brazilian Portuguese, German, French, Indonesian, Swedish, and Norwegian Bokmål each have 1,129 translatable strings + 5 plurals (1,134 names); no string arrays. English additionally contains the non-translatable `app_name`. This recount does not refresh the other snapshot inventories above |
...
The settings language picker is separate from DataStore. `AppLanguage` registers system default (`""`), English (`en`), Finnish (`fi`), Spanish (`es`), Brazilian Portuguese (`pt-BR`), German (`de`), French (`fr`), Indonesian (`id`), Swedish (`sv`), and Norwegian Bokmål (`nb`). `labelResId` supplies each language's native name; Indonesian is `Bahasa Indonesia`, Swedish is `Svenska`, and Norwegian Bokmål is `Norsk bokmål` in every catalog. `LanguageSelectionScreen` keeps system default first and sorts the remaining language rows using the UI locale's collator. Selection uses `AppCompatDelegate.getApplicationLocales()` and `setApplicationLocales(LocaleListCompat.forLanguageTags(...))`; AppCompat's `autoStoreLocales=true` metadata owns persistence on older Android versions. `AppLanguage.fromLocale` uses canonical BCP-47 tags for regional fallback, including Android versions whose `Locale.language` returns legacy `in` for Indonesian. Existing preference storage is unchanged.

`AndroidManifest.xml` points `android:localeConfig` to `res/xml/locales_config.xml`, with `en`, `fi`, `es`, `pt-BR`, `de`, `fr`, `id`, `sv`, and `nb`. Indonesian has one resource set, `values-in/strings.xml`: the installed SDK 37 `ResourcesImpl.adjustLanguageTag` maps modern `id` to legacy `in` at the Android resource boundary, and the project's cached AGP 9.4.0 AAPT2 accepts this qualifier. No duplicate `values-id` set is needed. Indonesian remains left-to-right. `MainActivity` handles `locale|layoutDirection` changes; bundle language splitting is disabled and no app resource-language filter is configured.
