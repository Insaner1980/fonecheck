...
| Present but not the routed implementation | `RunAllTestsViewModel` owns `RunAllTestsState`; completed data is a `DiagnosticReport`, stored in Room through the versioned `ReportPayloadCodec` payload. `CategoryTestResult`, `TestResult` and `TestStatus` remain active result-presentation adapters in `RunAllResultsScreen`; they must not be mistaken for unused scaffold or the durable payload. Empty `.gitkeep` files are placeholders, not implemented services. |
