...
| Item | Value |
|---|---|
| Verified source snapshot | 2026-09-10; live working tree including existing uncommitted and untracked changes |
| Verified Git baseline | `67dbb0ba40bb517bd82f748d300ed30d04b1b280` on `codex/fonecheck-architecture-follow-up`; the working tree was clean before this documentation edit. This reference describes the current working tree, not that commit alone. No remote fetch was performed. |
...
| Source inventory | 175 production Kotlin files plus 3 debug-only Kotlin files; 88 JVM-test Kotlin files (84 files and 469 annotations with `@Test`); 49 instrumented-test Kotlin files (48 files and 171 annotations with `@Test`) |
...
Polish plurals use `one`, `few`, `many`, `other`. All quantity-resource callers use integers. Static CLDR-rule checks and added Android assertions cover 0, 1, 2, 3, 4, 5, 10, 12, 14, 21, 22, 24, 25, 100, 101, 102 and 105; the Android tests also distinguish decimal `other` rules without claiming a decimal quantity-resource caller. A new `sensor_step_unit` plural selects the inflected word beside the live step count; existing locales retain their previous unit wording. PDF integer sample counts now use the existing `sensor_samples` plural through the presentation labels. Every earlier resource definition is preserved, with only the `Polski` choice and step-unit plural added. Persisted data, schemas, scores and comparison logic are unchanged.
...
| `app/src/test/` | Eighty-eight host-side JUnit policy, model, mapper, repository, formatting, navigation, concurrency, privacy, localization, and presentation test files. Four Kotlin files are shared fakes/fixtures; 84 files contain tests. |
| `app/src/androidTest/` | Forty-nine Android/Compose/Room/FileProvider/PDF integration, localization, cross-layer journey, navigation, and UI-semantics Kotlin files; 48 contain tests. These require an emulator or device and are not run by the ordinary JVM test task. |
