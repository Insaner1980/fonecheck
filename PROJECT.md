...
| Item | Value |
|---|---|
| Verified source snapshot | 2026-09-23 (Europe/Helsinki); tracked source at HEAD before this documentation edit |
| Verified Git baseline | `84087fd38908cd6ea3b445592295f43daf796502` on `codex/fonecheck-architecture-follow-up`. No remote fetch was performed; remote freshness and release status are unknown. |
| Initial Git/worktree state | 483 tracked paths with no staged or unstaged changes; 17 non-ignored untracked files under `.claude/` and `tmp/triage-004-runtime/` (500 tracked-plus-untracked paths). The untracked local settings and runtime evidence are preserved but excluded from implementation inventories. This document describes tracked HEAD, not those local artifacts. |
...
| Source inventory | 177 production Kotlin files plus 3 debug-only Kotlin files; 92 JVM-test Kotlin files (88 files and 570 literal `@Test` annotations); 59 instrumented-test Kotlin files (58 files and 214 literal `@Test` annotations). The included stability-analyzer build has separate source/tests. |
...
| Localization inventory | Thirteen shipped language sets: English, Finnish, Spanish, Brazilian Portuguese, German, French, Indonesian, Swedish, Norwegian Bokmål, Danish, Italian, Polish, and Turkish. Each has 1,143 translatable strings + 6 plurals (1,149 names); no string arrays. English additionally contains the non-translatable `app_name`. |
...
|---|---|
| `app/src/main/java/com/insaner/fonecheck/` | Production Kotlin: root `FonecheckApp.kt` plus `data` (8 files), `di` (6), `domain` (15), `export` (3), `localization` (2), `navigation` (4), `runtime` (3), and `ui` (135), for 177 files total. Empty `.gitkeep` placeholders are not Kotlin source. |
...
| `app/src/test/` | Ninety-two host-side JUnit policy, model, mapper, repository, formatting, navigation, concurrency, privacy, localization, and presentation Kotlin files. Four are shared fakes/fixtures; 88 files contain tests. |
| `app/src/androidTest/` | Fifty-nine Android/Compose/Room/FileProvider/PDF integration, localization, cross-layer journey, navigation, and UI-semantics Kotlin files; 58 contain tests and `localization/LocalizationTestAssertions.kt` is shared support code. These require an emulator or device and are not run by the ordinary JVM test task. |
...
The exact source-set count is deliberate: `C:\Dev\fonecheck\app\src\test\java` has 92 Kotlin files, 88 with `@Test` and 570 literal test annotations in total; the other four are `FakeReportRepository.kt`, `FakeAppPreferencesRepository.kt`, `testing/ReportFixtures.kt` and `testing/SequenceNanoTimeSource.kt`. `C:\Dev\fonecheck\app\src\androidTest\java` has 59 Kotlin files, 58 with tests and 214 literal `@Test` annotations; `LocalizationTestAssertions.kt` is the sole support-only Kotlin file in that source set. The included `tools/stability-analyzer` build has three separate Kotlin test files in its compiler/Gradle modules and is outside these app counts and the GitHub test job. Do not turn file/annotation counts, JaCoCo configuration, stability baselines, or Sonar exclusions into a test-pass, stability-pass, or coverage-quality claim without an actual revision-bound run and report inspection.
