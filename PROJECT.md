...
| `ReadoutWindow` + `WindowLabel`/`WindowReading`/`WindowRow`/`WindowBar` | The dark recess that holds the one reading a section exists to deliver. `WindowReading` pairs a figure with its unit and stacks them at a large font scale. Open a window only when one value honestly carries the section; a section whose meaning is a list of rows does not get one. Replaced `HeadlineReadout`, which was deleted once Full Check Results stopped using it. |
