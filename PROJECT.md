...
| Item | Value |
|---|---|
| Verified source snapshot | 2026-09-11; live working tree, which was clean before this documentation edit |
| Verified Git baseline | `9e7cb00ccdb22dd0cb72e9302a77026c37d32679` on `codex/fonecheck-architecture-follow-up`. This reference describes that clean branch tip plus this `PROJECT.md` update. No remote fetch was performed. |
...
| Source inventory | 175 production Kotlin files plus 3 debug-only Kotlin files; 89 JVM-test Kotlin files (85 files and 483 annotations with `@Test`); 54 instrumented-test Kotlin files (53 files and 185 annotations with `@Test`) |
...
| Localization inventory | Thirteen shipped language sets: English, Finnish, Spanish, Brazilian Portuguese, German, French, Indonesian, Swedish, Norwegian Bokmål, Danish, Italian, Polish, and Turkish. Each has 1,134 translatable strings + 6 plurals (1,140 names); no string arrays. English additionally contains the non-translatable `app_name`. |
...
`AndroidManifest.xml` points `android:localeConfig` to `res/xml/locales_config.xml`, with `en`, `fi`, `es`, `pt-BR`, `de`, `fr`, `id`, `sv`, `nb`, `da`, `it`, `pl`, and `tr`. Indonesian has one resource set, `values-in/strings.xml`: the installed SDK 37 `ResourcesImpl.adjustLanguageTag` maps modern `id` to legacy `in` at the Android resource boundary, and the project's cached AGP 9.4.0 AAPT2 accepts this qualifier. No duplicate `values-id` set is needed. Indonesian remains left-to-right. `MainActivity` handles `locale|layoutDirection` changes; bundle language splitting is disabled and no app resource-language filter is configured.

### Historical locale-specific review notes (non-authoritative)

The remaining notes in this subsection record prior static language-review work. They are retained as historical context only; they neither enumerate the current locale set nor establish a current build, device, rendering, accessibility, or editorial-quality result. The current registry/resource/test ownership is described in the Localization section.
...
Norwegian Bokmål uses one `values-nb/strings.xml` catalog and the `AppLanguage.NORWEGIAN_BOKMAL` registry entry. `nb` and `nb-NO` use the same language choice and generic `nb` formatting. The app adds no `no` alias and advertises no Nynorsk translation: `no`, `no-NO`, `nn`, and `nn-NO` remain unregistered and `AppLanguage.fromLocale` returns `SYSTEM` for those tags. The inspected AOSP resource matcher does not equate `no` or `nn` with `nb`, and `ResourcesImpl.adjustLanguageTag` does not rewrite them. Expected Android behavior is the next supported requested language, or English with no supported alternative; an explicit later `nb` preference can therefore select Bokmål. This is a source-level expectation, not device proof. ICU formatting inheritance between Norwegian locales is not Android resource-language registration. See [AOSP resource matching](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/libs/androidfw/ResourceTypes.cpp) and [language-tag adjustment](https://android.googlesource.com/platform/frameworks/base/+/refs/heads/main/core/java/android/content/res/ResourcesImpl.java).

The Bokmål glossary uses `Fullstendig sjekk`, `rapport`, `test på nytt`, `poeng`, `fullføringsgrad` (completed applicable evidence), `pålitelighet` (observation confidence), and `termisk margin` (the unitless estimate, with 1.0 explained as the severe-throttling threshold). `Ikke målt`, `utilgjengelig`, `støttes ikke`, `ikke relevant`, `hoppet over`, `avbrutt`, `tidsavbrudd`, and `tillatelse avslått` retain distinct meanings. Battery charge is `ladenivå`, capacity is `batterikapasitet`, and health is `batterihelse`; the headline says `Batteristatus fra Android`. Storage is `lagring` and working memory is `RAM`. All five plurals use `one` and `other` for the integer quantities supplied by current call sites, consistent with [CLDR Bokmål cardinal rules](https://unicode.org/cldr/charts/49/supplemental/language_plural_rules.html#nb). No production formatter, report payload, scoring, comparison, or JSON change was needed.

This addition received XML inventory/type/duplicate, placeholder, markup, and line-break checks across all nine catalogs, plus Bokmål URL, separator, locale-registration, and localization-reference checks; resource-only compilation using the cached AGP 9.4.0 AAPT2 (2.20-15978811); shipped-font glyph checks; and a separate linguistic pass over the complete Bokmål catalog. A local JDK 21 probe preserved all six Norwegian locale tags, produced comma decimals, U+00A0 grouping, U+2212 minus, and `11. aug. 2026, 12:18:00` for the summer Oslo sample. This host probe is not execution of the app formatters on Android. Existing JVM and Android tests were extended for Bokmål registration/non-claims, resource parity, fonts, formatting, all five plurals, diagnostic states/reasons/sources, accessibility, language switching/recreation, Home at 320 dp / 200% font scale, and PDF text/payload invariance. Project tests were not executed: no Gradle or app runtime was run. Pending runtime checks include `nb-NO` and `no-NO` resolution, both themes, long compounds such as `sikkerhetsoppdatering`, TalkBack, Android file sizes and dates, representative screens, and a rendered PDF with æ, ø, å and long evidence. No independent native-speaker review was performed; `termisk margin`, observation `pålitelighet`, and vibration `grunneffekter` merit review in context. The pre-existing legacy cancelled/skipped reason conflation described above remains unchanged. Other project areas were not reverified.
...
Italian uses one generic `values-it/strings.xml` catalog, registered as `AppLanguage.ITALIAN` with `it` in `locales_config.xml` and the native picker label `Italiano` in every catalog. Source-level selection maps `it`, `it-IT`, and `it-CH` to the same registry entry; normal Android resource matching is expected to use this generic catalog for both regions. The existing formatting policy normalizes all three to `it`, preserving the supplied/system time zone separately. There is no separately localized Italy or Switzerland catalog and no production formatter change. AppCompat persistence, system selection, English fallback, and the current-language PDF context are reused. Stored reports, evidence/reason IDs, scoring, coverage calculations, comparison, JSON, permissions, and layout are unchanged.

The Italian glossary uses `Controllo completo`, `rapporto`, `ripeti test` / `nuovo test`, `punteggio`, `completezza dei controlli` (completed applicable observations, including information), `affidabilità` (observation confidence), and `margine termico` (the unitless estimate approaching the severe-throttling threshold at 1.0). `Non misurato`, `non disponibile`, `non supportato`, `non applicabile`, `saltato`, `annullato`, `tempo scaduto`, and `autorizzazione negata` retain their distinct meanings. Battery charge is `livello di carica`, capacity is `capacità`, and health is `stato di salute della batteria`; the headline explicitly says `Stato batteria da Android`. Storage is `archiviazione` and working memory is `RAM`. Dynamic category retest titles use `Ripeti test: %1$s` without a gender-dependent article. All five plurals contain `one`, `many`, and `other` per [CLDR Italian cardinal rules](https://www.unicode.org/cldr/charts/48/supplemental/language_plural_rules.html#it). Current callers use integers: 1 selects `one`, 0 and 2 select `other`, and exact positive millions select `many` in recent ICU or `other` in older Android ICU. Decimal plural callers are absent.

The Italian addition received XML/UTF-8, name/type/duplicate, placeholder/precision/literal-percent, plural/array, markup, line-break, URL, Android-escaping, prohibited-separator, locale-registration, and production resource-reference checks. Coverage is 1,131 strings and five plurals, with no missing or extra Italian names and no string arrays. The ten earlier catalogs retain their previous content plus the native `Italiano` choice. Only source `app_name` is non-translatable; brands, technical units, native language names, and third-party license terms remain unchanged. All 78 fixed evidence labels and 55 stable-value mappings resolve to Italian resources. Resource-only AAPT2 2.20-15087165 compilation passed for all 11 string catalogs and the locale configuration without Gradle. A headless JDK 21 probe verified the Italian text and uppercase Italian accents in the four shipped text fonts and generic Italian number/date samples: `-1.234,5`, `12,5%`, `11 ago 2026, 12:18:00 UTC+02:00`, and winter `11 gen 2026, 11:18:00 UTC+01:00` for Rome and Zurich. This is host evidence, not execution of app formatters on Android.

A separate linguistic pass reviewed the complete Italian catalog for terminology, negation, grammar, elision, plurals, and accidental source-language carryover. No independent native-speaker review was performed; `margine termico`, diagnostic `affidabilità`, and the haptic primitive labels merit review in context. Existing JVM and instrumented tests were extended for resource parity, glyphs, selection, numbers/dates, all five plurals, diagnostic and accessibility states, regional fallback, Home layout, language switching/recreation, and PDF content/payload invariance. Older unsupported-language fixtures now use unregistered Japanese rather than newly supported Italian. Project tests were not executed, and no Gradle or app runtime was run. Pending checks: representative Italian screens, both regional tags, system and explicit language switching, long instructions, 320 dp width, 200% font scale, light/dark themes, TalkBack, Android file sizes/numbers/dates, and an Italian PDF with accents, apostrophes, and long evidence. The pre-existing legacy cancelled/skipped reason conflation described above remains unchanged; current observation reasons distinguish them. Other project areas were not reverified.

Danish uses one generic `values-da/strings.xml` catalog, registered as `AppLanguage.DANISH` with `da` in `locales_config.xml` and the native picker label `Dansk`. Both `da` and `da-DK` select the same registry entry in source and normalize to `da` in the existing formatting layer; Android resource resolution for Denmark is expected to use the generic catalog but has not been run on a device. System selection, AppCompat persistence, existing languages, locale-aware numbers/file sizes/dates, and the current-language PDF context are reused. No production formatter, layout, stored report payload, evidence/reason ID, score, coverage calculation, comparison, or JSON contract changed.

The Danish glossary uses `Fuld kontrol`, `rapport`, `test igen` / `ny test`, `point`, `gennemførelsesgrad` (completed applicable evidence, including information), `pålidelighed` (measurement/observation confidence), and `termisk råderum` (the unitless estimate whose 1.0 threshold means severe throttling). Battery charge is `ladeniveau`, capacity is `batterikapacitet`, and health is `batteritilstand`; the headline explicitly says `Batteristatus fra Android`. Storage is `lager` and working memory is `RAM`. `Ikke målt`, `ikke tilgængelig` / `utilgængelig`, `ikke understøttet`, `ikke relevant`, `sprunget over`, `annulleret`, `tidsgrænsen er nået`, and `tilladelse afvist` remain distinct. All five plurals use `one` and `other`, as required by [CLDR Danish cardinal rules](https://www.unicode.org/cldr/charts/48/supplemental/language_plural_rules.html#da); the current callers use integers, with 1 selecting `one` and 0, 2, and larger integers selecting `other`.

The Danish addition received XML/type/key/duplicate, placeholder, literal-percent, plural, markup, line-break, URL, separator, locale-registration, and source-reference checks. All 78 fixed evidence labels and 55 stable-value mappings have Danish resources. All nine earlier catalogs retain their previous contents plus the `Dansk` choice; only the source `app_name` remains non-translatable. Resource-only AAPT2 2.20-15978811 compilation passed for all ten string catalogs and the locale configuration without Gradle. A headless JDK 21 probe checked the four shipped text fonts and generic Danish number/date formatting: `-1.234,5`, `11. aug. 2026 12.18.00 UTC+02:00`, and the winter Copenhagen offset `+01:00`. This is host evidence, not Android formatting or PDF rendering proof. A separate linguistic pass reviewed every Danish entry, including compounds, grammatical gender, negation, and accidental Swedish/Bokmål/English. No independent native-speaker review was performed. The owner independently reviewed and confirmed `termisk råderum` and observation `pålidelighed`; these terminology questions are resolved. Inspection of `HapticDetails` and `readSupportedPrimitives` confirmed that the separate primitives list represents `VibrationEffect.Composition.PRIMITIVE_*` support via `arePrimitivesSupported`. Its Danish label is now `Understøttede haptiske grundelementer`, and the explanatory note uses `et haptisk grundelement`; predefined effects remain `effekter`. This terminology correction does not establish rendered layout or device validation.

JVM and instrumented tests were extended for Danish resource contracts, fonts, locale selection, numbers/dates, all five plurals, states/reasons/sources, accessibility, category-name insertion, language switching/recreation, existing-renderer PDF localization, Home layout, and PDF text/payload invariance. These project tests were not executed. No Gradle, app build, or device/emulator runtime was run. Pending runtime checks cover representative screens, language switching and system default, `da-DK`, long compounds such as `udviklerindstillinger`, 320 dp width, 200% font scale, both themes, accessibility labels/state descriptions, Android numbers/file sizes/dates, and a rendered Danish PDF with æ, ø, å and long evidence. The pre-existing legacy cancelled/skipped reason conflation described above remains unchanged. Other project areas were not reverified.
...
### Polish localization (2026-09-10)

Generic `pl` is registered as `AppLanguage.POLISH` with the native picker label `Polski` and one `values-pl` catalog. Source inspection establishes that both `pl` and `pl-PL` select the same language choice and normalize formatting to `pl`; Android resource resolution and switching are covered by added tests but were not run. The 1,134 strings and six plurals include all currently implemented diagnostic, accessibility, history, comparison, report and human-readable PDF labels. There are no missing or extra Polish names and no arrays. Only the English `app_name` is non-translatable. Technical formats, brands, units, language autonyms and third-party license terms remain intact.

Working terminology: Full Check = `Pełna diagnostyka`; diagnostic check = `test diagnostyczny`; report = `raport`; retest = `ponowny test`; score = `ocena`; coverage = `stopień ukończenia`; confidence = `wiarygodność`; thermal headroom = `margines termiczny`; battery health = `kondycja baterii`, explicitly `stan baterii według Androida` for the reported status; charge = `poziom naładowania`; capacity = `pojemność`; storage = `pamięć` / `pamięć wewnętrzna`; working memory = `RAM`. States distinguish `nie zmierzono`, `niedostępne`, `nieobsługiwane`, `nie ma zastosowania`, `pominięto`, `anulowano`, `przekroczono limit czasu`, `odmówiono uprawnienia`, `ostrzeżenie`, `wynik negatywny`, `informacja` and `potwierdzenie użytkownika`. Category placeholders use neutral constructions such as `Ponowny test: %1$s` to avoid inflecting arbitrary names. The thermal explanation retains the unitless severe-throttling threshold of 1.0 and explicitly says it is not a temperature.

Polish plurals use `one`, `few`, `many`, `other`. All quantity-resource callers use integers. Static CLDR-rule checks and added Android assertions cover 0, 1, 2, 3, 4, 5, 10, 12, 14, 21, 22, 24, 25, 100, 101, 102 and 105; the Android tests also distinguish decimal `other` rules without claiming a decimal quantity-resource caller. `sensor_step_unit` selects the inflected word beside the live step count, and PDF integer sample counts use `sensor_samples` through the presentation labels. The Polish introduction added the `Polski` picker choice and step-unit plural at that task boundary; a later six-locale correction changed only the singular step-unit words listed in the general Localization section above. Persisted data, schemas, scores and comparison logic are unchanged.

Executed for this change: Python UTF-8/XML inventory and resource-contract checks across all 12 catalogs (names/types, duplicates, nonempty values, placeholder types/precision/literal percents, locale-specific plural sets, arrays, markup, line breaks and URLs); Polish Android quote-escape, prohibited-separator and encoding checks; registry/configuration/qualifier checks; coverage of all 1,134 production Kotlin string references, including 226 distinct references in `EvidenceLocalization` and 61 in `ReportPdfRenderer`; and preservation checks against the task-start resource copies. These passed. Resource-only AAPT2 2.20-15087165 compilation passed for the 12 catalogs and locale configuration, without Gradle. A JDK 21.0.12 probe confirmed glyph availability for all Polish text and Polish uppercase letters in the four shipped Regular/Medium text fonts, and checked host-JDK decimal/integer/percentage formatting and Warsaw summer/winter dates for normalized `pl` and `pl-PL`. This is not Android formatting or PDF rendering evidence. The initial broader probe also uppercased technical units, turning the micro symbol into Greek capital mu; that unsupported transformation is not applied to those values in the app. The corrected probe checks the actual resource text and Polish uppercase letters.

The implementing assistant separately reviewed the full Polish inventory for language, inflection, negation, consistent terminology and diagnostic meaning. No independent native-speaker review was performed. The user independently reviewed and accepted margines termiczny and diagnostic wiarygodność; both are retained. The unitless thermal explanation remains unchanged. Confidence call sites present diagnostic reliability, not statistical probability; the existing heading and complete high/low labels do not benefit from adding Poziom. The haptic terminology follow-up below records the inspected mappings and narrow corrections. The existing legacy cancelled/skipped reason conflation remains unchanged; current observation reasons distinguish these states.

Added or extended tests cover registry/parity/glyphs, all six plurals, diagnostic states and reasons, locale-list priorities for every existing language, numbers/dates, camera formatting, Home layout, picker selection, language switching/recreation, PDF font glyphs/content, sample-count presentation and JSON/payload invariance. No Gradle, unit or instrumented tests, app runtime, rendered UI, TalkBack or rendered PDF checks ran. Pending: representative Polish screens, explicit/System language switching, `pl-PL`, plural counts 1/2/5/12/22/25, long labels/instructions at 320 dp and 200% font scale, both themes, accessibility labels/state descriptions, Android numbers/dates/file sizes and a Polish PDF containing ą ć ę ł ń ó ś ź ż with long evidence descriptions. The localization is implemented with static verification only. Other project areas were not reverified.


#### Polish haptic terminology follow-up (2026-09-10)

Inspected VibrationPlatform.kt, VibrationCapabilityPolicy.kt, VibrationTestScreen.kt and the Full Check evidence mapping. API argument order, enum order and localized label mapping agree. Predefined effects and composition primitives remain separate capability rows; shared CLICK/TICK sensations may have matching individual names without merging those concepts. Hardware detection and amplitude control remain separate. Constants, evidence IDs, reason codes and serialized values were not edited.

Individual meanings were checked against Android's [predefined effect reference](https://developer.android.com/reference/android/os/VibrationEffect) and [composition primitive reference](https://developer.android.com/reference/android/os/VibrationEffect.Composition). The [Polish haptics API guide](https://developer.android.com/develop/ui/views/haptics/haptics-apis?hl=pl) distinguishes effects, primitive building blocks and amplitude control. CLICK remains a click, TICK a light tick, THUD an impact, SPIN a rotational sensation, RISE upward motion and FALL downward motion; LOW_TICK explicitly identifies low frequency. These are contextual translation judgments, not an independent native-speaker certification.

Complete vibration/haptic resource inventory below includes controls, statuses and explanatory text. Keep evaluates the existing Polish wording; changed rows give both old and current text. A workflow label has no Android effect constant unless stated.

| Resource | Current Polish text | Source concept / Android mapping | Assessment and action |
| --- | --- | --- | --- |
| run_all_preflight_interactions | Sprawdzisz ekran i wykonasz obsługiwane testy aparatu, ruchu, przycisków głośności, wibracji, głośnika i biometrii. | Full Check manual-interaction overview, including vibration | Keep: natural and semantically appropriate in this context. |
| run_all_preflight_audio_vibration | Jeśli wybierzesz te testy, głośnik odtworzy krótki dźwięk, a mikrofon nagra krótką próbkę do odczytu względnego poziomu. Wibracje mogą być silne. | Full Check sound and vibration preflight warning | Keep: natural and semantically appropriate in this context. |
| run_all_vibration_title | Czy wibracja była wyczuwalna? | User perception question after Full Check playback | Keep: natural and semantically appropriate in this context. |
| run_all_vibration_description | fonecheck właśnie odtworzył krótki wzór wibracji. Wibracje mogą być silne. W każdej chwili możesz je zatrzymać, powtórzyć, potwierdzić lub pominąć. | Full Check playback and stop/retry/confirm/skip instructions | Keep: natural and semantically appropriate in this context. |
| run_all_felt_vibration | Wibracja wyczuwalna | Manual confirmation that vibration was felt | Keep: natural and semantically appropriate in this context. |
| run_all_no_vibration | Brak wibracji | Manual confirmation that vibration was not felt; not hardware detection | Keep: natural and semantically appropriate in this context. |
| home_cat_vibration | Wibracje | Vibration diagnostic category | Keep: natural and semantically appropriate in this context. |
| vibration_motor_title | Test silnika wibracji | Manual vibration motor test section | Keep: natural and semantically appropriate in this context. |
| vibration_short | Krótka | SHORT: createOneShot(100 ms, DEFAULT_AMPLITUDE) | Keep: natural and semantically appropriate in this context. |
| vibration_long | Długa | LONG: createOneShot(500 ms, DEFAULT_AMPLITUDE) | Keep: natural and semantically appropriate in this context. |
| vibration_pattern | Wzór | PATTERN: createWaveform([0,100,100,200,100,300], -1) | Keep: natural and semantically appropriate in this context. |
| vibration_did_you_feel | Czy wibracja była wyczuwalna? | User perception question; not an API capability | Keep: natural and semantically appropriate in this context. |
| vibration_yes | Tak | Manual FELT answer | Keep: natural and semantically appropriate in this context. |
| vibration_no | Nie | Manual NOT_FELT answer | Keep: natural and semantically appropriate in this context. |
| vibration_haptic_title | Możliwości haptyczne | Section containing Android-reported hardware, amplitude, effect and primitive capabilities | Keep: natural and semantically appropriate in this context. |
| vibration_amplitude_control | Sterowanie amplitudą | Vibrator.hasAmplitudeControl() | Keep: natural and semantically appropriate in this context. |
| vibration_effects_supported | Obsługiwane efekty | Vibrator.areEffectsSupported(EFFECT_*), retaining VIBRATION_EFFECT_SUPPORT_YES entries | Keep: natural and semantically appropriate in this context. |
| vibration_primitives_supported | Podstawowe elementy haptyczne | Vibrator.arePrimitivesSupported(Composition.PRIMITIVE_*), retaining true entries | Changed from Obsługiwane elementy składowe: The generic component label did not identify haptic primitives; the supported-list context supplies support. |
| vibration_has_vibrator | Silnik wibracji | Vibrator.hasVibrator(); hardware presence | Keep: natural and semantically appropriate in this context. |
| vibration_felt | Wyczuwalna | VibrationMotorResult.FELT | Keep: natural and semantically appropriate in this context. |
| vibration_not_felt | Niewyczuwalna | VibrationMotorResult.NOT_FELT | Keep: natural and semantically appropriate in this context. |
| vibration_skipped | Pominięto | VibrationMotorResult.SKIPPED | Keep: natural and semantically appropriate in this context. |
| vibration_none | Brak | Empty supported effect/primitive list; not a read failure | Keep: natural and semantically appropriate in this context. |
| vibration_stop | Zatrzymaj wibrację | Stops playback via Vibrator.cancel() | Keep: natural and semantically appropriate in this context. |
| vibration_skip | Pomiń potwierdzenie odczucia | Skips subjective perception confirmation | Keep: natural and semantically appropriate in this context. |
| vibration_unavailable | Android zgłasza, że to urządzenie nie ma silnika wibracji. | No vibration hardware reported by Android | Keep: natural and semantically appropriate in this context. |
| vibration_capability_read_error | fonecheck nie mógł odczytać wszystkich możliwości wibracji. Spróbuj ponownie otworzyć ekran. | Capability-query failure | Keep: natural and semantically appropriate in this context. |
| vibration_playback_error | fonecheck nie mógł uruchomić wibracji. Spróbuj ponownie. | Playback failure | Keep: natural and semantically appropriate in this context. |
| vibration_read_failed | Nie udało się odczytać | Failed capability read placeholder | Keep: natural and semantically appropriate in this context. |
| vibration_strength_warning | Długie wibracje i wzory wibracji mogą być silne lub nieprzyjemne. Możesz je zatrzymać w każdej chwili. | Long/pattern playback discomfort warning and stop instruction | Keep: natural and semantically appropriate in this context. |
| vibration_accessibility_note | Jeśli wibracja jest nieprzyjemna lub nie można jej wiarygodnie wyczuć, pomiń potwierdzenie odczucia. Obsługa zgłaszana przez API nadal jest podawana osobno. | Allows skipping perception while retaining separate API support reporting | Keep: natural and semantically appropriate in this context. |
| vibration_api_requires_android_11 | Wymaga Androida 11 lub nowszego | Application gate for support-query APIs (API 30), not the introduction level of every primitive | Keep: natural and semantically appropriate in this context. |
| vibration_capability_note | Obsługa przez API oznacza, że Android zgłasza natywny efekt lub podstawowy element haptyczny. Nie potwierdza to, że wibracja była fizycznie wyczuwalna. | API-reported effect/primitive support is separate from user-confirmed physical perception | Changed from Obsługa przez API oznacza, że Android zgłasza natywny efekt lub element składowy wibracji. Nie potwierdza to, że wibracja była fizycznie wyczuwalna.: Names primitives separately from effects; retains the physical-perception caveat. |
| vibration_effect_click | Kliknięcie | VibrationEffect.EFFECT_CLICK | Keep: natural and semantically appropriate in this context. |
| vibration_effect_double_click | Podwójne kliknięcie | VibrationEffect.EFFECT_DOUBLE_CLICK | Keep: natural and semantically appropriate in this context. |
| vibration_effect_heavy_click | Mocne kliknięcie | VibrationEffect.EFFECT_HEAVY_CLICK | Keep: natural and semantically appropriate in this context. |
| vibration_effect_tick | Lekkie stuknięcie | VibrationEffect.EFFECT_TICK | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_click | Kliknięcie | VibrationEffect.Composition.PRIMITIVE_CLICK | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_thud | Głuche uderzenie | VibrationEffect.Composition.PRIMITIVE_THUD | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_spin | Wirowanie | VibrationEffect.Composition.PRIMITIVE_SPIN | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_quick_rise | Szybkie wznoszenie | VibrationEffect.Composition.PRIMITIVE_QUICK_RISE | Changed from Szybkie narastanie: Narastanie suggested increasing intensity; Android describes fast upward motion. |
| vibration_primitive_slow_rise | Powolne wznoszenie | VibrationEffect.Composition.PRIMITIVE_SLOW_RISE | Changed from Powolne narastanie: Narastanie suggested increasing intensity; Android describes slow upward motion. |
| vibration_primitive_quick_fall | Szybkie opadanie | VibrationEffect.Composition.PRIMITIVE_QUICK_FALL | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_tick | Lekkie stuknięcie | VibrationEffect.Composition.PRIMITIVE_TICK | Keep: natural and semantically appropriate in this context. |
| vibration_primitive_low_tick | Stuknięcie o niskiej częstotliwości | VibrationEffect.Composition.PRIMITIVE_LOW_TICK | Changed from Niskie stuknięcie: Niskie stuknięcie was unnatural and did not identify low frequency. |
| observation_reason_user_confirmed_vibration_failure | Potwierdzenie użytkownika: podczas testu wibracja silnika nie była wyczuwalna. | Explanation of user-confirmed motor-test failure; reason code unchanged | Keep: natural and semantically appropriate in this context. |

### Turkish localization (2026-09-10, static validation only)

Turkish adds one generic `values-tr/strings.xml` catalog and `AppLanguage.TURKISH("tr", R.string.settings_language_turkish)`, with `tr` appended to `locales_config.xml`. The native picker label is `Türkçe` in all thirteen catalogs. Source-level locale selection and formatting normalize `tr` and `tr-TR` to the same Turkish language; Android resolution and switching remain unexecuted. No production formatter, presentation casing, report schema, persistence, scoring, or diagnostic behavior changed. The 1,134 strings and six plurals cover every current translatable name; there are no missing/extra names or arrays. Only English `app_name` is non-translatable. At the Turkish task boundary, earlier catalogs changed only by gaining the new picker label; later singular step-unit corrections are recorded in the general Localization section above.

Working glossary: Full Check = `Tam kontrol`; diagnostic check = `tanılama kontrolü`; report = `rapor`; retest = `yeniden test`; score = `puan`; diagnostic coverage = `tamamlanma oranı`; confidence = `güvenilirlik`; thermal headroom = `termal marj`; battery health = `pil sağlığı` (the displayed status is explicitly `Android pil durumu`); battery charge = `şarj düzeyi`; capacity = `pil kapasitesi`; storage = `depolama`; working memory = `RAM`. States retain `ölçülmedi`, `kullanılamıyor`, `desteklenmiyor`, `uygulanamaz`, `atlandı`, `iptal edildi`, `zaman aşımı`, `izin reddedildi`, `uyarı`, `başarısız`, `bilgi`, and `kullanıcı onayı`. Dynamic category titles use `Yeniden test: %1$s` without attaching a fixed suffix to arbitrary text. The thermal explanation explicitly describes a unitless 1.0 severe-throttling threshold, not a temperature.

All six plurals use `one` and `other`, following [CLDR Turkish cardinal rules](https://www.unicode.org/cldr/charts/48/supplemental/language_plural_rules.html#tr); current Android quantity callers supply integers. Nouns after explicit numbers remain singular (`2 örnek`, `5 kategori`, `10 gün`). Percentage resources preserve their escaped percent/positional argument contracts and place the sign before the number (`%12,5`). Haptic `low tick` is `Düşük frekanslı tık`, retaining the [Android primitive's low-frequency meaning](https://developer.android.com/reference/android/os/VibrationEffect.Composition#PRIMITIVE_LOW_TICK).

The casing audit found that `SectionHeader`, `StatusText`, and Home status labels already uppercase using `LocalLocale.current.platformLocale`. Section headers and status text retain natural spoken labels; trailing section values are not uppercased. Technical Kotlin no-argument `lowercase()` calls are invariant, explicit `Locale.ROOT` normalizers stay unchanged, and the unknown-code titlecase fallback operates on technical codes. The English PDF-label factory is not the live renderer's Turkish label source: the renderer supplies localized labels from a fresh app-language context for each render. Added JVM tests cover `İzin ve ışık` / `İZİN VE IŞIK`, all four I forms and stable `info`/`fail` codes under a Turkish default. Added Compose assertions exercise real shared components in every registered locale and `tr-TR`, preserving natural accessibility wording.

Executed without Gradle: Python UTF-8/XML parsing and resource inventory/type/duplicate/placeholder/markup/line-break/plural/separator/URL/Android-apostrophe checks across all thirteen catalogs, registry/qualifier and localization-reference checks, and existing-language preservation checks. All passed. Direct cached AAPT2 resource compilation of the Turkish catalog passed. A JDK 21.0.12 probe confirmed Turkish upper/lowercase, counts, negative/grouped decimals, prefix percentages, and Istanbul summer/winter date/time behavior for `tr` and `tr-TR`. Java AWT confirmed all Turkish resource text plus uppercase titles/category/status labels and Turkish letters in the four shipped Regular/Medium text fonts. This is host-JDK/glyph evidence, not Android resource resolution, UI rendering, or PDF glyph/layout proof. The implementing assistant separately reviewed the complete Turkish catalog for language and semantics; no independent native-speaker review occurred. The user subsequently reviewed these terminology areas; the narrow resolution below replaces that pending terminology note. This does not claim an independent native-speaker review of the entire catalog.

Added/extended JVM and Android tests were not executed: resource contracts, locale choice, numbers/dates, technical JSON invariance, Turkish states/reasons/sources/PDF labels/plurals/file sizes, casing, Home fit, picker selection, switching and recreation. Pending runtime validation: representative Turkish onboarding/Home/14 diagnostic/Full Check/history/report/comparison/export screens, language switching and `tr-TR`, dotted/dotless I and uppercase headers, long labels/instructions at 320 dp and 200% font scale in both themes, accessibility labels/state descriptions, Turkish numbers/dates and a PDF containing `ç ğ ı İ ö ş ü` with long evidence descriptions. Existing legacy cancelled/skipped reasons still share one presentation resource; current observation reasons distinguish them. Unknown future codes and raw vendor/platform text remain technical fallbacks.

### Turkish terminology resolution (2026-09-10)

The user independently reviewed the flagged terms. Diagnostic `tamamlanma oranı` is retained: current coverage resources describe completion of applicable diagnostic evidence. Both former Turkish `ısıl pay` uses (`thermal_headroom_title` and `thermal_status_note`) refer to the `ThermalPlatform` -> `PowerManager.getThermalHeadroom()` value; the same title maps durable `thermal.headroom` evidence in reports/PDF. `Isı payı` is shorter but can suggest heat; `Isıl pay` was understandable and internally consistent. `Termal marj` is concise and reads naturally beside the existing unitless-threshold explanation, so it is selected following the user's preference. The [Turkish Thermal API guide](https://developer.android.com/games/optimize/adpf/thermal?hl=tr) uses multiple variants; this decision does not claim one uniquely official label. No Celsius meaning or calculation changes are introduced.

Final title: **Termal marj**. The explanation remains exactly:

> 1,0 değeri, cihazın güçlü ısıl kısıtlama eşiğidir. Bu birimsiz tahmin bir sıcaklık değeri değildir ve desteklenen Android sürümlerinde bile kullanılamayabilir.

The related status note now reads:

> Bu, Android tarafından bildirilen mevcut performans kısıtlama durumudur. Cihaza özgü ısıl tasarım, durum ile termal marj arasındaki ilişkiyi etkileyebilir.

The [Turkish haptics guide](https://developer.android.com/develop/ui/views/haptics/haptics-apis?hl=tr) distinguishes predefined effects, composition building blocks (`temel öğeler`), amplitude control and hardware. Under the existing `Dokunsal özellikler` section, the primitive row becomes `Desteklenen temel öğeler`; its caveat uses `haptik temel öğe`. The same resource labels serve persisted evidence presentation. Individual names retain click, heavy/double click, light tick, low-frequency tick, impact, spin, rising and falling sensations according to the [Composition reference](https://developer.android.com/reference/android/os/VibrationEffect.Composition). No API constants, evidence IDs, reason codes or serialized values change. The motor playback buttons use one-shot/waveform effects; the listed EFFECT_* and PRIMITIVE_* entries are capability queries, not playback buttons.

Complete Turkish vibration/haptic resource mapping, including guidance and related Full Check labels:

| Resource | Final Turkish text | Actual concept | Decision |
|---|---|---|---|
| `run_all_vibration_title` | Titreşimi hissettiniz mi? | Full Check user perception question | Retained |
| `run_all_vibration_description` | fonecheck az önce kısa bir titreşim deseni oynattı. Desenli titreşim güçlü hissedilebilir; istediğiniz zaman durdurabilir, tekrarlayabilir, onaylayabilir veya atlayabilirsiniz. | Full Check patterned playback guidance and safety | Retained |
| `run_all_felt_vibration` | Hissettim | Full Check user confirms perception | Retained |
| `run_all_no_vibration` | Titreşim yok | Full Check user reports no perception | Retained |
| `home_cat_vibration` | Titreşim | Vibration diagnostic category | Retained |
| `vibration_motor_title` | Motor testi | Motor test with user confirmation; not an API capability verdict | Retained |
| `vibration_short` | Kısa | VibrationPattern.SHORT: createOneShot(100 ms, DEFAULT_AMPLITUDE) | Retained |
| `vibration_long` | Uzun | VibrationPattern.LONG: createOneShot(500 ms, DEFAULT_AMPLITUDE) | Retained |
| `vibration_pattern` | Desen | VibrationPattern.PATTERN: createWaveform, non-repeating on/off pattern | Retained |
| `vibration_did_you_feel` | Hissettiniz mi? | User perception question | Retained |
| `vibration_yes` | Evet | User confirms felt vibration | Retained |
| `vibration_no` | Hayır | User confirms vibration not felt | Retained |
| `vibration_haptic_title` | Dokunsal özellikler | Haptic capability section, including hardware, amplitude, effects and primitives | Retained |
| `vibration_amplitude_control` | Genlik kontrolü | Vibrator.hasAmplitudeControl() | Retained |
| `vibration_effects_supported` | Desteklenen efektler | Vibrator.areEffectsSupported(): predefined EFFECT_* with SUPPORT_YES | Retained |
| `vibration_primitives_supported` | Desteklenen temel öğeler | Vibrator.arePrimitivesSupported(): Composition.PRIMITIVE_* | Changed: use the primitive terminology family |
| `vibration_has_vibrator` | Titreşim motoru | Vibrator.hasVibrator() | Retained |
| `vibration_felt` | Hissedildi | VibrationMotorResult.FELT user observation | Retained |
| `vibration_not_felt` | Hissedilmedi | VibrationMotorResult.NOT_FELT user observation | Retained |
| `vibration_skipped` | Atlandı | Skipped subjective motor confirmation | Retained |
| `vibration_none` | Yok | Empty supported effects/primitives list, not a hardware absence assertion | Retained |
| `vibration_stop` | Titreşimi durdur | Vibrator.cancel() action | Retained |
| `vibration_skip` | Fiziksel onayı atla | Skip subjective motor confirmation | Retained |
| `vibration_unavailable` | Android bu cihazda titreşim motoru olmadığını bildiriyor. | Hardware absent according to hasVibrator() | Retained |
| `vibration_capability_read_error` | fonecheck tüm titreşim özelliklerini okuyamadı. Ekranı yeniden açmayı deneyin. | Hardware/amplitude/effect/primitive capability query failure | Retained |
| `vibration_playback_error` | fonecheck titreşimi başlatamadı. Yeniden deneyin. | Vibration playback could not start | Retained |
| `vibration_read_failed` | Okunamadı | Unavailable capability value after query failure | Retained |
| `vibration_strength_warning` | Uzun ve desenli titreşimler güçlü veya rahatsız edici olabilir. İstediğiniz zaman durdurun. | Safety notice for long or patterned motor playback | Retained |
| `vibration_accessibility_note` | Titreşim rahatsız ediyorsa veya güvenilir biçimde algılanamıyorsa fiziksel onayı atlayın. API desteği ayrı olarak bildirilmeye devam eder. | Skip unreliable/uncomfortable perception; API evidence remains separate | Retained |
| `vibration_api_requires_android_11` | Android 11+ gerekir | Existing API 30 support-query gate, not the introduction level of every primitive | Retained |
| `vibration_capability_note` | API desteği, Android tarafından yerel bir efekt veya haptik temel öğe bildirildiği anlamına gelir. Titreşimin fiziksel olarak hissedildiğini doğrulamaz. | Native API support distinguished from physical user perception | Changed: use the primitive terminology family |
| `vibration_effect_click` | Tıklama | VibrationEffect.EFFECT_CLICK | Retained |
| `vibration_effect_double_click` | Çift tıklama | VibrationEffect.EFFECT_DOUBLE_CLICK | Retained |
| `vibration_effect_heavy_click` | Güçlü tıklama | VibrationEffect.EFFECT_HEAVY_CLICK | Retained |
| `vibration_effect_tick` | Hafif tık | VibrationEffect.EFFECT_TICK | Retained |
| `vibration_primitive_click` | Tıklama | VibrationEffect.Composition.PRIMITIVE_CLICK | Retained |
| `vibration_primitive_thud` | Tok vuruş | VibrationEffect.Composition.PRIMITIVE_THUD | Retained |
| `vibration_primitive_spin` | Dönme | VibrationEffect.Composition.PRIMITIVE_SPIN | Retained |
| `vibration_primitive_quick_rise` | Hızlı yükseliş | VibrationEffect.Composition.PRIMITIVE_QUICK_RISE | Retained |
| `vibration_primitive_slow_rise` | Yavaş yükseliş | VibrationEffect.Composition.PRIMITIVE_SLOW_RISE | Retained |
| `vibration_primitive_quick_fall` | Hızlı düşüş | VibrationEffect.Composition.PRIMITIVE_QUICK_FALL | Retained |
| `vibration_primitive_tick` | Hafif tık | VibrationEffect.Composition.PRIMITIVE_TICK | Retained |
| `vibration_primitive_low_tick` | Düşük frekanslı tık | VibrationEffect.Composition.PRIMITIVE_LOW_TICK | Retained |
| `observation_reason_user_confirmed_vibration_failure` | Test sırasında titreşim motorunun hissedilmediğini onayladınız. | Stable observation reason for user-confirmed missing perception | Retained |

### Historical localization notes (non-authoritative)

The notes below record earlier, language-specific static-review work. They do not describe the current thirteen-language implementation or establish current validation; live resource directories, `AppLanguage`, `locales_config.xml`, and test sources above are the authority. Known source-level caveats that remain relevant are that Camera capability strings retain the locale at ViewModel creation, shared Camera/PDF technical strings use dash separators, and legacy `EvidenceReasonCode.CANCELLED` and `SKIPPED` share a presentation resource even though current observation reasons distinguish them. Unknown future stable codes and raw vendor/platform values intentionally remain outside the translation inventory.
...
The unit-test source set contains 89 Kotlin files: 85 contain 483 `@Test` annotations; `app/src/test/java/com/insaner/fonecheck/data/repository/FakeReportRepository.kt`, `app/src/test/java/com/insaner/fonecheck/data/preferences/FakeAppPreferencesRepository.kt`, `app/src/test/java/com/insaner/fonecheck/testing/ReportFixtures.kt`, and `app/src/test/java/com/insaner/fonecheck/testing/SequenceNanoTimeSource.kt` are support code. In addition to domain scoring/assembly/scope-aware comparison, persistence, policies, formatting, localization and Full Check mapping, the source cases pin comparison metadata versus value changes, evidence-aware camera labels and PDF local-time offset/semantics wording; `CameraCaptureSession` retake confirmation, timeout/cancel/late-callback behavior; shared Battery/Thermal temperature normalization; Bluetooth adapter-name exclusion through report, repository payload/JSON, and PDF presentation; Polish/Turkish locale behavior; and the per-category Full Check completion-review gate, stale-token rejection, and final acknowledgement before Results.

The instrumented-test source set contains 54 Kotlin files: 53 contain 185 `@Test` annotations and `localization/LocalizationTestAssertions.kt` is shared support code. Its source cases cover Room/DAO/repository/FileProvider/PDF and Compose/navigation behavior, including `JourneyEvidenceTest`, `JourneyLifecycleTest`, `JourneyPresentationTest`, `ReportRetestNavigationTest`, and `ReportRetestActionTest`. Together those files encode production-callback-to-report identity, category-retest persistence/Home/export continuity, lifecycle interruption without an accidental save/resume, saved-result scope/time presentation, fresh consecutive retest back-stack/ViewModel ownership, save-gated actions, and localized action layout at 200% font scale. Dedicated Polish and Turkish resource/casing tests cover their represented locale contracts; `StageCompletionScreenTest` pins a failed result on screen until Continue is pressed, and `ButtonCheckStepTest` pins both detected button rows plus the explicit Continue action. `HomeViewModelTest` separately covers empty state, latest-Full-Check selection against newer category retests, and summary-flow failure. File and annotation counts describe source inventory, not distinct runtime scenarios or a pass result. No test, coverage report, build, or Sonar scan was run for this documentation change; hardware/device coverage remains required for camera, audio, GPS/GNSS, sensors, Bluetooth, biometrics, vibration, volume keys, storage conditions, API-26 behavior, locale restart/configuration behavior, and release R8/signing.
...
| `app/src/test/` | Eighty-nine host-side JUnit policy, model, mapper, repository, formatting, navigation, concurrency, privacy, localization, and presentation Kotlin files. Four are shared fakes/fixtures; 85 files contain tests. |
| `app/src/androidTest/` | Fifty-four Android/Compose/Room/FileProvider/PDF integration, localization, cross-layer journey, navigation, and UI-semantics Kotlin files; 53 contain tests and `localization/LocalizationTestAssertions.kt` is shared support code. These require an emulator or device and are not run by the ordinary JVM test task. |
...
The exact source-set count is deliberate: `C:\Dev\fonecheck\app\src\test\java` has 89 Kotlin files, 85 with `@Test` and 483 test annotations in total; the other four are the fake repositories and shared test fixtures/time source named above. `C:\Dev\fonecheck\app\src\androidTest\java` has 54 Kotlin files, 53 with tests and 185 `@Test` annotations; `LocalizationTestAssertions.kt` is the sole support-only Kotlin file in that source set. Do not turn file/annotation counts, JaCoCo configuration, stability baselines, or Sonar exclusions into a test-pass, stability-pass, or coverage-quality claim without an actual revision-bound run and report inspection.
