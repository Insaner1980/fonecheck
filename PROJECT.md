...
- The root `osv-scanner.toml`, beside `buildscript-gradle.lockfile`, time-bounds `GHSA-r937-wjx7-w2jp` to 2026-09-30 because the affected KAPT incremental cache is unused: this project uses KSP. The exception must be removed or revalidated when Kotlin/processing configuration changes; its stated stable-Kotlin follow-up is configuration context, not proof that an update has been evaluated.
