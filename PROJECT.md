...
| Source inventory | 183 production Kotlin files plus 3 debug-only Kotlin files; 97 JVM-test Kotlin files (93 files and 599 literal `@Test` annotations); 61 instrumented-test Kotlin files (60 files and 220 literal `@Test` annotations). The included stability-analyzer build has separate source/tests. |
...
The committed `tools/benchmarks/NUMBER_FORMAT_RESULTS.md` records a direct Kotlin/JVM benchmark that compared the then-working-tree formatting change against baseline `1d726f5b7b9148cc93bef79d0fa86fbe4e58c6c7`. Results are medians across three independent JVM forks:
...
The Git inventory counts tracked paths and non-ignored untracked paths separately; production and test-source counts come from the four app source sets and exclude local untracked artifacts and the included build. Test counts are literal `@Test` annotations, not parameterized executions, assertions, scenarios or passes. Resource counts come from XML element names/types and exclude `translatable="false"`; all thirteen catalogs have identical translatable name/type sets and no duplicate resource names. Ignored build output, scanner databases and installed tool packages are outside those counts.

The earlier 2026-09-23 audit inspected the catalog/routes, diagnostic ownership, Full Check, reports, shell, resources, tests, configuration and local wrappers. The 2026-09-26 correction refreshes inventories after the PDF/network changes; it is not a repeat of that full audit. Assertions below describe implementation or configuration unless explicitly identified as test contracts, requirements or historical results. Existing GitHub results were read for PR head `7f3fef4`; no local test, checker, benchmark, external upload, build, emulator or device operation was executed. Release state and external policy pages were not checked.
...
References to “implemented” mean present in tracked HEAD, not released, compiled in this task, or verified on hardware. The untracked local runtime artifacts are not treated as product source or as new verification by this audit.

## Product state

fonecheck is a local, single-activity phone diagnostics app. It provides fourteen standalone diagnostic categories, guided Full Check sessions, immutable local reports, history, comparison, PDF/JSON export, first-use Home guidance, settings, and third-party notices. It has no account system, backend, analytics SDK, billing, cloud sync, network speed test, or INTERNET permission.
...
| Settings | Theme and app-language selection, test-warning toggle, concise Free/Full information, permission snapshot, report deletion, licenses, privacy/support handoff | No account, privacy-sync, or cloud setting exists. |
...
| Kotlin / AGP | Kotlin 2.4.10; Android Gradle Plugin 9.4.1; JVM 17 |
...
| Navigation | Navigation Compose 2.10.1 and type-safe Serializable routes |
| DI | Hilt 2.60.1 via KSP 2.3.12; AndroidX Hilt lifecycle ViewModel Compose 1.4.0 provides `hiltViewModel()` integration |
| Data | Room 2.8.5; DataStore Preferences 1.2.1; kotlinx.serialization JSON 1.11.0 |
...
| Static checks | ktlint Gradle plugin 14.2.0 with ktlint engine 1.8.0; Detekt 2.0.0-alpha.6; Compose Rules 0.6.6; locally patched Compose Stability Analyzer 0.13.0; Android Security Lints 1.0.4; OWASP Dependency-Check 13.0.0; SonarQube Gradle plugin 7.5.0.8588 |
| Tests | JUnit 4.13.2, kotlinx-coroutines-test 1.11.0, AndroidX Test Runner 1.7.0, AndroidX Test Ext JUnit 1.3.0, Compose UI test through the BOM, Room testing 2.8.5; debug unit-test coverage is enabled for JaCoCo/Sonar import |
| Security automation | CodeQL action v4.38.0, Semgrep 1.171.0, OSV-Scanner 2.4.0, project-local DeepSec 2.3.9, dependency verification metadata/keyring, buildscript dependency locking |
...
The root build pins selected buildscript transitives: Jackson 2.22.2, protobuf 4.36.1, jose4j 0.9.6, Bouncy Castle 1.85, JDOM 2.0.6.1, and jsoup 1.23.2. The app's `androidLintTool` configuration additionally forces Apache HttpClient 4.5.14, Commons Lang 3.20.0, and Bouncy Castle 1.85; these are lint-tool dependencies, not app runtime dependencies. The root build enables buildscript dependency locking. `gradle/verification-metadata.xml`, `gradle/verification-keyring.keys`, `buildscript-gradle.lockfile`, and `settings-gradle.lockfile` are supply-chain inputs and must move with intentional dependency/plugin or catalog-resolution updates. The verification metadata retains earlier artifacts and includes AGP 9.4.1 AAPT2 `9.4.1-15978811` artifacts for Linux and Windows. Their configured checksums/trust entries were inspected; remote artifacts and digests were not downloaded or independently verified in this audit. Only buildscript/settings lockfiles are present; do not describe these as a locked application runtime dependency graph.
...
`settings.gradle.kts` includes the repository-owned `tools/stability-analyzer` composite twice: for Gradle plugin resolution and to substitute its compiler artifact. Its isolated `0.13.0-fonecheck-patch1` implementation patches analyzer 0.13.0 receiver/signature reporting and comparison while runtime dependencies remain at upstream 0.13.0. `app/build.gradle.kts` selects this local plugin and pins analyzer-before-Compose compiler-plugin order so the dump sees source-level signatures. The patch records extension-receiver type and stability, uses qualified name plus receiver and ordered parameter types as a canonical JSON-encoded overload identity, rejects unexpected collisions, and handles ambiguous legacy baselines explicitly rather than assigning invented receivers. Read `tools/stability-analyzer/README.md` and its compiler/Gradle tests before changing the patch, plugin order, baseline format, or removing the composite. These source contracts and checked-in tests are not evidence that a stability task passed at this HEAD.

`app/stability/app-debug.stability` and `app/stability/app-release.stability` are variant baselines. Validation is configured to ignore non-regressive changes, and `compileDebugKotlin`/`compileReleaseKotlin` explicitly declare the shared config as an input because the analyzer plugin does not otherwise invalidate AGP's built-in Kotlin tasks when that file changes. `config/android-check.json` names `:app:stabilityCheck`, but the GitHub `build-test-lint` job does not currently invoke it; the included build's own test tasks are also outside that workflow. Any stability-config, patch or baseline update therefore needs a deliberate local/manual stability run and diff review; a baseline change is not automatically an optimization.
...
`runtime/AndroidBatteryHealthMapping.kt` and `runtime/AndroidThermalStatusMapping.kt` translate framework integer constants into stable domain codes. Battery/Performance/Thermal standalone classification and Full Check mapping share those functions, so an unknown vendor/framework value remains `OTHER` or `UNAVAILABLE` rather than being inferred as a good state. Their host tests cover the named constants and unknown values; device-specific meanings still need device evidence.
...
| data/preferences/ | Theme, warning, and dismissible Home-introduction DataStore preferences. |
...
`MainActivity` is an `@AndroidEntryPoint AppCompatActivity`. AppCompat is required by the cross-version per-app language path; the activity still installs AndroidX SplashScreen before `super.onCreate`, enables edge-to-edge, observes preferences lifecycle-aware, selects light/dark/system theme, and hosts one `NavHostController`. While the first DataStore value is unavailable, Compose renders only the housing plus a horizontally inset, clipped panel shell; the NavHost is created after preferences load so theme, warnings, and first-use guidance begin from one observed preference snapshot.
...
The start destination is always Home. When the persisted Home-introduction flag is false, Home renders a short, non-blocking introduction above its normal report/category content; the category launcher remains usable before dismissal. Dismissal reuses the released `onboarding_complete` DataStore key for compatibility, so users who completed the former tutorial do not see the replacement introduction. A failed dismissal remains retryable without blocking Home.

| Route group | Destinations |
|---|---|
| App / diagnostics | Home, contextual Full information, Device, Performance, SIM, Display, Audio, Camera, Sensors, Connectivity, Battery, Thermal, Storage, Vibration, Buttons, Biometrics |
...
`HomeViewModel` observes the Home-introduction preference separately from the latest-report flow. It ignores duplicate dismissal while a write is active, hides the introduction after a successful write, and restores a retryable inline error after a failed write. Fullscreen state is driven by the Display/Full Check disposable effects and `ConfigureSystemBars`; there is no route-change `LaunchedEffect` resetting it, which would compete with the entering Display effect.
...
| `ProgressWindow` | A polite text-and-`WindowBar` sequence indicator used by Full Check manual stages. Determinate progress is always a window bar, never a Material `LinearProgressIndicator`. |
...
| Connectivity | Wi-Fi, Bluetooth, NFC, GPS/GNSS, mobile network. | Location/phone/Bluetooth permissions gate portions; no Internet speed test exists. A denied fine-location grant remains explicit `permission_denied` GPS evidence when the captured state reports GPS available. |
...
| SIM | `inventory`, `network`, `base_network`, `network_display`, conditional `display_base_network` and `display_override`, plus one `slot_<zero-based-index>_state` item per reported slot. The four new network metadata IDs do not contribute to score, coverage or category aggregation. |
| Display | `info`, `visual` |
...
The planner de-duplicates stages and always appends Results. A category-only retest can therefore skip the general Automatic stage entirely unless its own plan needs it (Audio with microphone enabled also needs Automatic as a prerequisite); UI and cleanup logic must follow `RunAllPlan`, never assume the full sequence. Permission review derives its five possible prompts from the selected category and preflight choices, in microphone/camera/location/phone/Bluetooth order. A category retest with no relevant runtime permission automatically resolves that stage, while a full run still presents the review even if a particular optional permission is irrelevant.
...
| Home introduction dismissed | DataStore, legacy key `onboarding_complete` | `false` | Controls only the non-blocking first-use guidance on Home; it never changes the start route or locks Home actions. |
...
| Preferences | Theme, warning toggle, Home-introduction dismissal | Stored only in app-private DataStore, not in reports/exports. |
...
`ReportDetailPresenter` presents only stored categories. Saved detail reuses `RunAllResultsScreen(mode = SAVED_REPORT)`, including its score window. Result categories are grouped into attention, completed and incomplete; the first attention category initially expands. The UI filters through known destination mappings without fabricating categories absent from the report. Routed category retests use `RunAllTestsScreen` and `RunAllTestsViewModel`, with `RunAllSnapshotMapper` supplying snapshots, `ReportAssembler` creating the report, and `ReportRepository.insertOrConfirm` confirming persistence.
...
PDF uses Android `PdfDocument` on white A4-sized paper with bundled DM Sans and JetBrains Mono fonts. `ReportPdfContentBuilder` reads saved score/coverage, scope and metadata, creates bounded key findings, a saved-order category overview, grouped evidence and interpretation notes. `AndroidPdfLayout` measures and draws the same `StaticLayout` instances; `PdfLayoutEngine` paginates measured rows, keeps observations together when they fit, and repeats category/column headings on continuation pages. The PDF-only logo reuses the splash vector paths and original orange, with dark lettering and explicit viewport-padding compensation. Locale and time zone are captured per export. Every saved evidence entry remains represented, including network metadata that is excluded from coverage. JSON, scoring, storage and export finalization are unchanged by the redesign. See `docs/PDF_EXPORT.md` for the implementation boundary and user-run verification. GitHub reports successful debug/release builds and JVM tests for PR head `7f3fef4`; production-rendered PDF visual acceptance remains unverified. File finalization remains a checked rename/replace sequence, not a claimed crash-durable fsync transaction.
...
Static XML enumeration in this audit found identical translatable name/type sets in all thirteen catalogs: 1,207 strings and 6 plurals, with no missing, extra or duplicate names and no string arrays. This is resource inventory, not execution of `ResourceParityTest`, translation approval, AAPT compilation or device fallback proof.
...
Manual Gradle command references are `./gradlew assembleDebug`, `./gradlew assembleRelease`, and `./gradlew test`; CI uses the more specific `:app:assembleDebug`, `:app:assembleRelease`, `:app:testDebugUnitTest`, `:app:lintDebug`, and `:app:dependencyCheckAnalyze` tasks, while the local `build-check` configuration names only `:app:assembleDebug`. Relevant configured local gates additionally include `:app:ktlintCheck`, `:app:detekt`, and `:app:stabilityCheck`. The included stability build separately defines `:compose-stability-compiler:test`, `:compose-stability-gradle:test`, and report-audit tasks documented in its README. These are references, not evidence that current HEAD passed them.
...
| `app/src/test/` | Ninety-seven host-side JUnit policy, model, mapper, repository, formatting, navigation, concurrency, privacy, localization, and presentation Kotlin files. Four are shared fakes/fixtures; 93 files contain tests. |
| `app/src/androidTest/` | Sixty-one Android/Compose/Room/FileProvider/PDF integration, localization, cross-layer journey, navigation, and UI-semantics Kotlin files; 60 contain tests and `localization/LocalizationTestAssertions.kt` is shared support code. These require an emulator or device and are not run by the ordinary JVM test task. |
...
| `tools/stability-analyzer/` | Included composite build that owns the locally patched stability compiler/Gradle plugin, receiver-aware report format and comparison, isolated tests, and patch-removal criteria. Consult its README before changing plugin order or stability baselines. |
...
|---|---|
| Build/version/stability | `settings.gradle.kts`, `build.gradle.kts`, `app/build.gradle.kts`, `gradle.properties`, `gradle/libs.versions.toml`, `gradle/wrapper/gradle-wrapper.properties`, `gradle/gradle-daemon-jvm.properties`, `buildscript-gradle.lockfile`, `settings-gradle.lockfile`, `gradle/verification-metadata.xml`, `gradle/verification-keyring.keys`, `config/compose-stability.conf`, `app/stability/app-debug.stability`, `app/stability/app-release.stability`, `tools/stability-analyzer/README.md` |
...
| DI/runtime/permissions | `app/src/main/java/com/insaner/fonecheck/di/`, `app/src/main/java/com/insaner/fonecheck/runtime/RuntimeProviders.kt`, `app/src/main/java/com/insaner/fonecheck/runtime/AndroidBatteryHealthMapping.kt`, `app/src/main/java/com/insaner/fonecheck/runtime/AndroidThermalStatusMapping.kt`, `app/src/main/java/com/insaner/fonecheck/domain/permission/PermissionPolicy.kt`, `app/src/main/java/com/insaner/fonecheck/ui/permissions/PermissionController.kt`, `app/src/main/java/com/insaner/fonecheck/ui/ExternalActivityLauncher.kt` |
| Domain | `app/src/main/java/com/insaner/fonecheck/domain/model/DiagnosticEvidence.kt`, `app/src/main/java/com/insaner/fonecheck/domain/model/DiagnosticEvidenceStructure.kt`, `app/src/main/java/com/insaner/fonecheck/domain/model/DiagnosticReport.kt`, `app/src/main/java/com/insaner/fonecheck/domain/model/ReportAssembler.kt`, `app/src/main/java/com/insaner/fonecheck/domain/model/ScoreCalculator.kt`, `app/src/main/java/com/insaner/fonecheck/domain/observation/DeviceObservationClassifier.kt`, `app/src/main/java/com/insaner/fonecheck/domain/comparison/ReportComparisonEngine.kt` |
...
| Full Check | `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllTestsViewModel.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllTestsScreen.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllStagePlanner.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllPermissionRelevance.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllPermissionPrompts.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllSnapshotMapper.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllResourceOwner.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllResultsScreen.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllAutomaticChecks.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllStorageCheck.kt`, `app/src/main/java/com/insaner/fonecheck/ui/screens/runall/RunAllManualSteps.kt` |
| Contextual Full information | `app/src/main/java/com/insaner/fonecheck/ui/screens/access/FullAccessScreen.kt`; registered fallback route and localized Free/Full explanation, while `DiagnosticDestination.kt` currently enables direct Full access. |
...
|---|---|---|---|
| Home | Owns the text header and instrument face, shows dismissible first-use guidance, opens History/Settings and the latest saved Full Check, and exposes every catalog destination through one status panel. Automatic Full access opens every category and Full Check directly. | `LatestFullCheckState` explicitly represents loading, empty, available, corrupt/unsupported, and observation/load error; retry restarts summary observation. `HomeIntroductionState` independently represents visible, dismissing, and retryable dismissal-failure states. Only a real stored Full Check can populate the readout and lamps; access labels never replace diagnostic status. | Shared top bar is hidden only for Home. The latest-report readout stacks below 312 dp or above font scale 1.3. The status panel chooses two columns only when the longest localized category name fits; otherwise one. No duplicate category-row layer exists, and the introduction does not intercept Home actions. |
| Contextual Full information | Explains the selected Full-only category or Full Check, states the one-time €6.99 price and lack of subscription, and identifies the four Free categories. | There is no billing or entitlement implementation; automatic Full access bypasses this fallback surface and does not manufacture a diagnostic status. | `FullAccess(featureId)` remains registered for a future entitlement implementation and always offers a return path to Free functionality. |
...
| Settings | Changes theme and AppCompat app language, toggles test warnings, keeps concise app-purpose and Free/Full scope/price information discoverable, presents permission snapshot rows, deletes all reports after confirmation, opens licenses, and hands privacy/support to external apps. | `SettingsState` combines DataStore preferences, report count/deletion state, permission snapshot, and errors. App language is read/written separately through `AppCompatDelegate`. Permission rows inform; they do not replace contextual feature permission flows. | `SettingsRoute` owns locale selection, `ACTION_VIEW`, `ACTION_SENDTO`, app Settings, and navigation callbacks. `SettingsScreen.kt` divides appearance, Full information, permissions, reports, privacy/support, and about/version through shared section/row/rule primitives. |
...
| Compose/unit UI | Navigation chrome and initial destination, Home introduction/report selection/access/status policy, observation-to-presentation adapters, screen-state/theme/number/date/separator/localization helpers | A passing JVM run verifies the represented pure UI decisions, state mapping and routes. | Pixel-perfect layouts or AppCompat locale behavior on actual devices. |
| Included analyzer build | Three Kotlin test files under `tools/stability-analyzer/compose-stability-compiler` and `compose-stability-gradle` exercise receiver capture, overload/collision handling and report-format comparison. | A passing isolated included-build run would validate the tested patch contracts. | The current app's debug/release stability reports, baseline compatibility, or CI coverage; those require their separate tasks. |
| Instrumented UI and journeys | Home first-use guidance/explicit states/instrument readout/access/status panel/navigation/semantics/theme/font-scale/RTL, contextual Full information, settings/licenses/app-language selection, permission/state components, Full Check preflight/results, history/detail/comparison/export, report-retest navigation/actions, cross-layer evidence/lifecycle/presentation journeys, display interaction, PDF exporter, Room tests | A passing instrumented run verifies the represented Android runtime, Compose semantics, persisted identity and navigation/lifecycle paths on the device or emulator used. | Complete API-level, form-factor, TalkBack, locale-restart and physical-hardware coverage. |
...
The exact source-set count is deliberate: `C:\Dev\fonecheck\app\src\test\java` has 97 Kotlin files, 93 with `@Test` and 599 literal test annotations in total; the other four are `FakeReportRepository.kt`, `FakeAppPreferencesRepository.kt`, `testing/ReportFixtures.kt` and `testing/SequenceNanoTimeSource.kt`. `C:\Dev\fonecheck\app\src\androidTest\java` has 61 Kotlin files, 60 with tests and 220 literal `@Test` annotations; `LocalizationTestAssertions.kt` is the sole support-only Kotlin file in that source set. The included `tools/stability-analyzer` build has three separate Kotlin test files in its compiler/Gradle modules and is outside these app counts and the GitHub test job. Do not turn file/annotation counts, JaCoCo configuration, stability baselines, or Sonar exclusions into a test-pass, stability-pass, or coverage-quality claim without an actual revision-bound run and report inspection.

The current regression sources include `AudioOperationGateTest` for atomic admission/publication under cancellation and `RunAllVibrationReplayTest` for a failed replay terminating the stage as ERROR without inventing a measured motor failure. `ReportRetestNavigationTest` additionally exercises duplicate Home navigation, stale Back and outgoing-screen refresh disposal. Other cases cover cancelled information-read failure, restarted benchmark, backwards report clock, monotonic thermal interval, SIM subscription fallback, Home-introduction dismissal/access routing, GPS permission-denial provenance, and scrollable state/display/export behavior. The included analyzer has receiver/overload/report-format tests. These are source-inspected intended assertions, not newly observed passes.
...
3. If the local stability-analyzer patch, compiler-plugin order or baseline format changes, do receiver and overload identities still remain unambiguous in both new dumps and legacy comparisons?
4. Is `targetSdk = 36` still an intentional, reviewed compatibility boundary, and has the separately required Android 17/SDK 37 target pass been completed before changing or retaining the `OldTargetApi` suppression?
5. Does a Sonar coverage exclusion correspond to code that is genuinely exercised through instrumented/device evidence, or is it merely hidden from the JVM coverage denominator?
6. Was any Sonar or external-AI upload explicitly approved with scope/token/data-retention awareness, and is a local PlanOnly/configuration check being kept distinct from an uploaded clean result?
7. Are Dependabot configuration, repository enablement, alerts, generated PRs, and actual merges reported as separate states rather than one “enabled and clean” claim?
8. Does the CI Dependency-Check result cover only debug/release runtime classpaths, and were both remaining time-bounded suppressions validated for the exact resolved component rather than copied forward mechanically?

## Implemented, scaffolded, planned, and non-claim boundaries

| Boundary | Current repository evidence |
|---|---|
| Implemented | Fourteen category routes, explicit Free/Full Home access labels, non-blocking first-use Home guidance, contextual Full information, Full Check, immutable report domain, Room v1 storage, history/detail/retest/comparison, JSON/PDF export, settings, licenses, thirteen shipped language resources, tests and CI configuration. |
...
| Present but not the routed implementation | `RunAllTestsViewModel` owns `RunAllTestsState`; completed data is a `DiagnosticReport`, stored in Room through the versioned `ReportPayloadCodec` payload. `CategoryTestResult` and `TestResult` remain active result-presentation adapters in `RunAllResultsScreen`; their status fields use `DiagnosticStatus` directly, and the screen groups category adapters by that status. They must not be mistaken for unused scaffold or the durable payload. Empty `.gitkeep` files are placeholders, not implemented services. |
...
8. Compose stability is configured and baseline-backed but is not part of the current GitHub workflow; the local analyzer patch and its isolated tests are also outside CI. Review the shared stability contract, patch behavior and both variant baselines during release verification.
