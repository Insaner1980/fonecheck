...
|---|---|---|---|
| Home | Owns the text header and instrument face, shows dismissible first-use guidance, opens History/Settings and the latest saved Full Check, and exposes every catalog destination through one status panel. Free categories open directly; Full-only categories and Full Check open contextual Full information. | `LatestFullCheckState` explicitly represents loading, empty, available, corrupt/unsupported, and observation/load error; retry restarts summary observation. `HomeIntroductionState` independently represents visible, dismissing, and retryable dismissal-failure states. Only a real stored Full Check can populate the readout and lamps; access labels never replace diagnostic status. | Shared top bar is hidden only for Home. The latest-report readout stacks below 312 dp or above font scale 1.3. The status panel chooses two columns only when the longest localized category name fits; otherwise one. No duplicate category-row layer exists, and the introduction does not intercept Home actions. |
| Contextual Full information | Explains the selected Full-only category or Full Check, states the one-time €6.99 price and lack of subscription, and identifies the four Free categories. | There is no billing or entitlement implementation; this surface has no purchase action and does not manufacture a diagnostic status. | `FullAccess(featureId)` is entered only after an explicit Home selection and always offers a return path to Free functionality. |
