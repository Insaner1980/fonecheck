...
| Item | Value |
|---|---|
| Verified source snapshot | 2026-09-03 |
| Verified Git baseline | `cae4e57ab6447524e8ccb583b000f0b8571ba484` on `codex/android-toolchain-update`; the working tree was clean before this documentation-only update |
...
| Source inventory | 167 production Kotlin files plus 1 debug-only preview Kotlin file; 75 JVM-test Kotlin files (71 files and 316 annotations with `@Test`); 29 instrumented-test Kotlin files (all 29 files, 93 annotations with `@Test`) |
...
| Durable/config inventory | 14 catalog categories, 78 localized durable evidence IDs, 70 stable observation reasons, 14 manifest permissions, 13 optional manifest features |
| Localization inventory | English: 1,089 strings + 5 plurals; Finnish: 1,088 strings + 5 plurals; both contain the same 1,093 translatable resource names; English additionally contains only the non-translatable `app_name` |
...
| Ownership | This correction updates only `PROJECT.md` and `CODE_REVIEW.md`; no source, resource, test, schema, build, CI, checker, or generated file is changed. |
...
`NavigationChrome` centrally provides localized title/back state and whether the shared top bar is shown. Home sets `showTopBar = false` because `HomeContent` owns its header/actions; all mapped diagnostic, Full Check, report, history, comparison, export, onboarding, Settings, and Licenses routes retain the shared top bar. The shared top bar is a panel-coloured `TopAppBar` followed by `InstrumentTickRule`; navigation and optional refresh actions use `IconBoxButton`. A screen can register one `TopBarAction`; `MainActivity` resets it when the current route changes, and `RegisterRefreshTopBarAction` removes its registration on disposal. There is no top-edge fade or screen-local mask in the current shell.
...
| `DataRow` | Sans label, right-aligned monospace value, and hairline rule. The value is measured first and capped at `rowValueMaxWidth` (200 dp); the label receives the remaining width. The row stacks above font scale 1.3 or when the value exceeds its cap. `value = null` always renders the muted unavailable label and ignores verdict tone. Optional confidence and long-press copy remain inside the row. Ellipsised measured data is a defect. |
...
The exact source-set correction is deliberate: `C:\Dev\fonecheck\app\src\test\java` has 75 Kotlin files, 71 with `@Test` and 316 test annotations in total; the other four are the fake repositories and shared test fixtures/time source named above. `C:\Dev\fonecheck\app\src\androidTest\java` has 29 Kotlin files, all with tests, and 93 test annotations in total. Do not turn file/annotation counts, JaCoCo configuration, stability baselines, or Sonar exclusions into a test-pass, stability-pass, or coverage-quality claim without an actual revision-bound run and report inspection.
