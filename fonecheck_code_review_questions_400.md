...
AI can hallucinate or overstate issues. Verify every claim against the actual repository, relevant official Android API behavior when needed, tests, configuration, and available runtime evidence. If there is no genuine problem, say so clearly and do not invent or force a finding. For a verified issue, report the exact evidence, concrete failure scenario, severity, smallest safe correction, and focused tests or manual checks; do not edit source, tests, documentation, assets, configuration, reports, generated baselines, or schemas unless the user separately requests implementation. Keep review verification read-only: do not run Gradle, Sonar, external-AI scanning, or project checker execution; if a checker plan is useful, invoke its wrapper only with `-PlanOnly`. Never open a pull request.
```
...
Review recent abstractions, mappers, use cases, repositories, and wrappers for duplicated pass-through layers. Recommend removal or simplification only where they create a demonstrated bug, maintenance hazard, or testing obstacle; do not propose refactoring working code to match a preferred architecture.
...
Inspect existing tests and source paths for interruption, configuration changes, screen disposal, Activity recreation, stage timeout, and late effects. Look for a stale fullscreen callback that rehides bars or leaves content laid out for the wrong inset state.
...
Search production Composables for Cards with near-identical shape, border, elevation, padding, or container behavior. Recommend replacement only for concrete inconsistent duplicates that should use `StandardCard`; preserve specialized interaction surfaces with a documented reason.
...
Search for hard-coded sp values, copied font weights, manual line heights, and letter spacing. Report only genuine hierarchy or accessibility inconsistencies, not harmless specialized display values with a clear purpose.
...
Review start/end versus left/right padding, icons with directional meaning, checkmark/arrow mirroring, touch grid coordinates, comparison arrows, and numeric/technical values. Report layout-direction bugs that would break future localization or accessibility testing.
...
Check that each state names the situation and offers only actions that can work now, such as retry, request, Settings recovery, Back, or no action. Recommend removing disabled-looking primary buttons that have no outcome only when they are a real usability defect.
...
Inspect existing test coverage and source behavior for categories containing only unavailable or non-applicable evidence, including empty-after-filter cases. Ensure they do not aggregate to INFO, PASS, NOT_TESTED, or produce a score.
...
Inspect existing test coverage and source behavior for non-even averages, empty scoreable sets, large counts, and ordering. Confirm integer division floors rather than rounds and no INFO, unavailable, not-tested, or non-applicable evidence enters the mean.
...
Inspect existing test coverage and source behavior for exact values around 0, 69.x, integer ratios, and category/overall summaries. Confirm UI, entity metadata, comparison, and exports never display a numeric score when coverage state is INCOMPLETE.
...
Inspect existing test coverage and source behavior for threshold boundaries and mixed statuses. Confirm partial coverage retains a score only under the documented rules and is labeled consistently in UI and exports.
...
Inspect existing test coverage and source behavior for full reports with category IDs, category reports without IDs, unknown IDs, blank IDs, and corrupted payloads. Confirm constructors, entity validation, codec, and repository reject inconsistent combinations.
...
Inspect existing test coverage and source behavior for absent camera, microphone capability, telephony, NFC, GPS, vibration, biometrics, and optional sensors. Confirm the planner does not schedule impossible stages and the category remains present with specific stable reasons.
...
Review every stage `LaunchedEffect` or lifecycle effect and the exact claim key. Inspect existing test coverage for repeated composition, navigation transitions, state restoration, and multiple collectors to ensure an operation starts once per valid token.
...
Inspect existing test coverage and source behavior for full-report rows with a category, category-only rows without one, unknown stable IDs, null/blank values, and corrupted payload disagreement. Ensure repository reconstruction does not trust one side while ignoring the other.
...
Inspect existing test coverage and source behavior for COMPLETE/PARTIAL/INCOMPLETE states with null, negative, over-100, and valid scores. Verify entity invariants, payload validation, summary mapping, UI, comparison, and exports agree.
...
Inspect DAO ordering when timestamps tie, category retests and full reports share a time, IDs differ, or clock fakes repeat values. Recommend a stable secondary order only if nondeterminism causes a real user/test defect.
...
Inspect existing test coverage and source behavior for higher, lower, zero, negative, missing, or unknown schema values. Confirm History can still list a safe summary if designed to, while detail/comparison/export never deserialize unsupported content as current.
...
Inspect existing test coverage and source behavior for system, light, dark, unknown/corrupt values, rapid changes, process recreation, and write failure. Confirm UI reflects the actual persisted or fallback state without optimistic drift.
...
Inspect existing test coverage and source behavior for PASS/WARNING/FAIL/INFO/NOT_AVAILABLE/NOT_TESTED transitions and applicable/non-applicable changes. Confirm a status delta is not shown as a numeric improvement when the check became unavailable.
...
Inspect existing test coverage and source behavior for schema mismatch independently from score-version match. Ensure UI explains the limitation and does not calculate percentages from evidence models with potentially different semantics.
...
Review English/Finnish labels, dates, numbers, units, long values, page breaks, status colors plus text, null score, coverage limitation, all fourteen categories, and category-only reports. Check whether existing tests inspect generated PDF content rather than only string helpers.
...
Verify provider XML, authority string, cache directory, PDF resources/fonts, serializer, and Intent construction in a minified signed artifact. Recommend keep/resource rules only when an observed release failure justifies them.
...
Map phone state, microphone, audio settings, camera, coarse/fine location, Wi-Fi/network state, legacy Bluetooth, BLUETOOTH_CONNECT, NFC, vibration, and biometrics to concrete source paths. Report only truly unused or overbroad declarations after checking API-specific implicit needs.
...
Search for phone number, IMEI, MEID, IMSI, ICCID, subscriber ID, line number, voicemail number, and raw subscription identifiers in state, logs, Room, JSON, PDF, and UI. Report only actual unnecessary collection.
...
Inspect existing test coverage and source behavior for wired/Bluetooth headset connection changes, earpiece availability, focus loss, calls/alarms, mute/Do Not Disturb limitations, and route callbacks. Ensure interruption becomes an explicit test state rather than false failure.
...
Inspect callback frequency, state updates, live regions, warning transitions, and debounce if needed. Report only actual responsiveness or accessibility flooding problems.
...
Inspect bytes, milliseconds/seconds, hertz, cores, SIMs, sensors, reports, warnings/failures, cycles, percentages, and dimensions. Report concatenated English only when localization or grammar is genuinely broken.
...
Inspect revision-bound output from the narrowest appropriate clean or assemble task when available, but do not run the task during review-only work. Do not infer success from configuration, generated files, unrelated prior logs, documentation, or the existence of tests.
...
Derive the current unit-test file and annotation inventory from the working tree, distinguish test classes from support code, and never present those counts as discovery, execution, coverage, or a pass result. Report incorrect count automation only if it is actually used.
...
### 381. Is instrumented-test source inventory kept separate from discovery and execution evidence?

```text
Review the fonecheck implementation for this specific concern: Is instrumented-test source inventory kept separate from actual device/emulator discovery and execution evidence?
...
Review status priority, applicability, null scores, exact 70/100 coverage thresholds, floor means, duplicate/missing categories, category reports, versions, and invalid evidence. Recommend only missing cases that can catch a plausible defect.
...
Inspect selectors, test tags, visible text, roles, state descriptions, live regions, and responsive grids. Recommend replacing brittle hierarchy/index assertions only when they cause real fragility or miss user-observable behavior.
...
Inspect existing test coverage and source behavior below 600, exactly 600, 839, exactly 840, and representative wider sizes with realistic density and font scale. Confirm two/three/four columns and spacing behavior.
...
Inspect revision-bound build/install smoke-test evidence for the actual release-like artifact, covering Hilt startup, Room, serialization, all routes, CameraX, biometrics, PowerProfile fallback, PDF/JSON export, FileProvider, licenses, and localization. Recommend broad keep rules only when that evidence demonstrates a failure.
